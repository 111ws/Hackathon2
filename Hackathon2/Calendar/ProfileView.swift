...
struct ProfileView: View {
	...
	var body: some View {
		NavigationStack {
			...
			.background(WarmTheme.primaryBackground)
			.navigationTitle("")
		}
	}
	...
	private func header() -> some View {
		HStack(alignment: .center, spacing: 16) {
				...
				HStack(spacing: 8) {
					...
					if isPro {
						Text("Pro")
							...
							.padding(.vertical, 2)
							.background(WarmTheme.lightAccent.opacity(0.3), in: Capsule())
					}
				}
				Text(joinedText)
					.font(.footnote)
			...
			Spacer()
			Circle()
				.fill(WarmTheme.lightAccent.opacity(0.4))
				.frame(width: 72, height: 72)
				.overlay(
					Circle()
						.stroke(WarmTheme.accent, lineWidth: 2)
				)
		}
	}
